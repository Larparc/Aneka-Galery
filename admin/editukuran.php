...
if (isset($_POST['add_size'])) {
    ...
    if (!empty($name)) {
        ...
        if ($stmt->execute()) {
            ...
            $message = "Gagal menambahkan ukuran: " . $conn->error;
            $messageType = "error";
        }
        $stmt->close();
    } else {
        $message = "Nama ukuran tidak boleh kosong.";
        $messageType = "error";
    }
    header("Location: editukuran.php?msg=" . urlencode($message) . "&type=" . $messageType);
    exit;
}
...
if (isset($_POST['edit_size'])) {
    ...
    if ($id > 0 && !empty($name)) {
        ...
        if ($stmt->execute()) {
            ...
            $message = "Gagal memperbarui ukuran: " . $conn->error;
            $messageType = "error";
        }
        $stmt->close();
    } else {
...
if (isset($_GET['delete_size'])) {
    ...
    if ($id > 0) {
        ...
        if ($stmt->execute()) {
            ...
            $message = "Gagal menghapus ukuran: " . $conn->error;
            $messageType = "error";
        }
        $stmt->close();
    } else {
...
// Ambil data ukuran
$sql = "SELECT size_id, size_name FROM sizes ORDER BY size_id ASC";
...
<head>
    <meta charset="UTF-8">
    <title>Ukuran - Aneka Galery</title>
    <link rel="shortcut icon" href="../img/anekagalery_32x32.png">
    <link rel="stylesheet" href="../css/panel.css">
    <link rel="stylesheet" href="../css/editukuran.css">
...
<div class="app">
    ...
    <aside class="sidebar" id="sidebar">
        <div class="logo">
            ...
            <div><a href="../index.php"><b>ANEKA GALERI</b><span>Digital Printing</span></a></div>
        </div>
        <nav>
            <a href="panel.php">
    ...
    <div class="main">
        <header class="topbar">
            <div class="user">
                ...
                <div class="user-avatar-wrapper"><img src="../img/foto.jpg" class="user-avatar" alt="avatar"></div>
                    ...
                    <?php if (count($sizes) > 0): ?>
                        ...
                        <tr>
                            ...
                            <td class="actions" style="text-align:center;">
                                <button class="btn-action edit" data-id="<?= $item['size_id'] ?>" data-name="<?= htmlspecialchars($item['size_name']) ?>">
                                    <i class="fas fa-edit"></i> Edit
                                </button>
                                <button class="btn-action delete" data-id="<?= $item['size_id'] ?>" data-name="<?= htmlspecialchars($item['size_name']) ?>">
    ...
    <div class="modal-box">
        ...
        <form method="POST" action="">
            ...
            <div class="form-group">
                ...
                <input type="text" id="addName" name="size_name" placeholder="Masukkan nama ukuran" required>
            </div>
            <div class="form-group">
                <label>Type</label>
                <div class="readonly-input">Ukuran</div>
            </div>
...
<div class="modal-overlay" id="editModal">
    <div class="modal-box">
        ...
        <h2>Edit Ukuran</h2>
        <form method="POST" action="">
            <input type="hidden" name="edit_size" value="1">
            <input type="hidden" id="editId" name="size_id">
            <div class="form-group">
                <label for="editName">Name</label>
                <input type="text" id="editName" name="size_name" placeholder="Masukkan nama ukuran" required>
            </div>
            <div class="form-group">
                <label>Type</label>
                <div class="readonly-input">Ukuran</div>
...
<div class="modal-overlay" id="deleteModal">
    <div class="modal-box" style="max-width: 420px;">
        ...
        <p style="margin-bottom: 16px; font-size: 15px;">Apakah Anda yakin ingin menghapus ukuran ini?</p>
        <div style="background: #f8f9fa; padding: 12px 16px; border-radius: 8px; margin-bottom: 20px;">
            ...
            <div><strong>Type:</strong> Ukuran</div>
        </div>
        <div class="modal-actions">
            <button type="button" class="btn btn-cancel" id="cancelDelete">Cancel</button>
            <a href="#" id="confirmDeleteBtn" class="btn btn-danger" style="padding: 10px 24px; border-radius: 30px; font-weight: 600; font-size: 14px; text-decoration: none;">Confirm & Delete</a>
        </div>
    </div>
</div>
...
<script>
    ...
    document.querySelectorAll('.btn-action.delete').forEach(btn => {
        btn.addEventListener('click', function() {
            ...
            confirmDeleteBtn.href = '?delete_size=' + id;
